        ...
        switch (bg->GetTypeID(true))                             // Custom values
        {
            case BATTLEGROUND_RB:
                switch (bg->GetMapId())
                {
                    ...
                    case 727:                                   // SM
                        data->WriteBits(0x00000001, 24);
                        buff << uint32(((BattlegroundSMScore*)itr->second)->CartsControlled);      // carts controlled
                        break;
                    case 761:                                   // BG
                        data->WriteBits(0x00000002, 24);
                    ...
                    case 998:                                   // TK
                        data->WriteBits(0x00000002, 24);
                        buff << uint32(((BattlegroundTKScore*)itr->second)->OrbHandles);           // Orb handles
                        buff << uint32(((BattlegroundTKScore*)itr->second)->VictoryPoints);        // Orb victory points
                        break;
                    case 1105:                                  // DG
                        data->WriteBits(0x00000004, 24);
                        buff << uint32(((BattlegroundDGScore*)itr->second)->CartsCaptures);        // carts captures
                        buff << uint32(((BattlegroundDGScore*)itr->second)->CartsReturned);        // carts returned
                        buff << uint32(((BattlegroundDGScore*)itr->second)->MinesAssaulted);       // bases assaulted
                        buff << uint32(((BattlegroundDGScore*)itr->second)->MinesDefended);        // bases defended
                        break;
                    default:
                        data->WriteBits(0, 24);
                        break;
...
Battleground* BattlegroundMgr::CreateNewBattleground(BattlegroundTypeId originalBgTypeId, PvPDifficultyEntry const* bracketEntry, uint8 arenaType, bool isRated)
{
    ...
    switch (bgTypeId)
    {
        ...
        case BATTLEGROUND_TA:
            ...
            break;
        case BATTLEGROUND_TG:
            bg = new BattlegroundTG(*(BattlegroundTG*)bg_template);
            break;
        case BATTLEGROUND_RB:
        case BATTLEGROUND_AA:
