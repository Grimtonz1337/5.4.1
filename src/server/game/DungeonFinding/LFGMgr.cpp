...
void LFGMgr::LoadLFGDungeons(bool reload /* = false */)
{
    ...
    for (uint32 i = 0; i < sLFGDungeonStore.GetNumRows(); ++i)
    {
        ...
        switch (dungeon->type)
        {
            ...
            case LFG_TYPE_DUNGEON_RANDOM:
                LfgDungeonStore[dungeon->ID] = LFGDungeonData(dungeon);
                break;
            case LFG_TYPE_RAID:
            case LFG_TYPE_RAID_RANDOM:
                LfgDungeonStore[dungeon->ID] = LFGDungeonData(dungeon);
                break;
            case LFG_TYPE_SCENARIO:
            case LFG_TYPE_SCENARIO_RANDOM:
                LfgDungeonStore[dungeon->ID] = LFGDungeonData(dungeon);
                break;
        }
    }
    ...
    for (LFGDungeonContainer::iterator itr = LfgDungeonStore.begin(); itr != LfgDungeonStore.end(); ++itr)
    {
        ...
        if (dungeon.type != LFG_TYPE_DUNGEON_RANDOM || LFG_TYPE_RAID_RANDOM || LFG_TYPE_SCENARIO_RANDOM && dungeon.x == 0.0f && dungeon.y == 0.0f && dungeon.z == 0.0f)
        {
        ...
        if (dungeon.type != LFG_TYPE_DUNGEON_RANDOM || LFG_TYPE_RAID_RANDOM || LFG_TYPE_SCENARIO_RANDOM)
...
{
    if (!isOptionEnabled(LFG_OPTION_ENABLE_DUNGEON_FINDER | LFG_OPTION_ENABLE_RAID_BROWSER | LFG_OPTION_ENABLE_RAID_FINDER | LFG_OPTION_ENABLE_SCENARIO_FINDER))
        return;
        ...
        for (LfgDungeonSet::const_iterator it = dungeons.begin(); it != dungeons.end() && joinData.result == LFG_JOIN_OK; ++it)
        {
            ...
            switch (type)
            {
                case LFG_TYPE_DUNGEON_RANDOM:
                    ...
                    // No break on purpose
                case LFG_TYPE_RAID_RANDOM:
                    if (dungeons.size() > 1)               // Only allow 1 random raid
                        joinData.result = LFG_JOIN_DUNGEON_INVALID;
                    else
                        rDungeonId = (*dungeons.begin());
                    // No break on purpose
                case LFG_TYPE_SCENARIO_RANDOM:
                    if (dungeons.size() > 1)               // Only allow 1 scenario dungeon
                        joinData.result = LFG_JOIN_DUNGEON_INVALID;
                    else
                        rDungeonId = (*dungeons.begin());
                    // No break on purpose
...
void LFGMgr::FinishDungeon(uint64 gguid, const uint32 dungeonId)
{
    ...
    for (LfgGuidSet::const_iterator it = players.begin(); it != players.end(); ++it)
    {
        ...
        if (!dungeon || (dungeon->type != LFG_TYPE_DUNGEON_RANDOM && !dungeon->seasonal))
        {
...
bool LFGMgr::selectedRandomLfgDungeon(uint64 guid)
{
    if (GetState(guid) != LFG_STATE_NONE)
    {
        ...
        if (!dungeons.empty())
        {
             ...
             if (dungeon && (dungeon->type == LFG_TYPE_DUNGEON_RANDOM || LFG_TYPE_RAID_RANDOM || LFG_TYPE_SCENARIO_RANDOM || dungeon->seasonal))
                 return true;
        }
    }
...
LfgDungeonSet LFGMgr::GetRandomAndSeasonalDungeons(uint8 level, uint8 expansion)
{
    ...
    for (lfg::LFGDungeonContainer::const_iterator itr = LfgDungeonStore.begin(); itr != LfgDungeonStore.end(); ++itr)
    {
        ...
        if ((dungeon.type == lfg::LFG_TYPE_DUNGEON_RANDOM || LFG_TYPE_RAID_RANDOM || LFG_TYPE_SCENARIO_RANDOM || (dungeon.seasonal && sLFGMgr->IsSeasonActive(dungeon.id)))
