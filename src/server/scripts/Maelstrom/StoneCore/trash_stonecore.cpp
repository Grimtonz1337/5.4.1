...
#define spell_shadow_fury 92644
#define spell_shadow_bolt 92641
#define spell_frostbolt_volley 92642
...
class mob_millhouse_manastorm : public CreatureScript
{
 public:
	  ...
	  {
	    mob_millhouse_manastormAI(Creature * pCreature) : ScriptedAI(pCreature) {}
		...
		void Reset()
		{
		    fear = urand(5000, 8000);
			shadow_fury = 10000;
			shadow_bolt = 3000;
			frostbolt_volley = urand(12000, 15000);
		}
		...
		void UpdateAI (const uint32 diff)
	   {
			   ...
			   if (fear<= diff)
               {
               DoCast(SelectTarget(SELECT_TARGET_RANDOM,1,100,true), spell_fear);
               fear = urand(5000, 8000);
               } else fear-= diff;
   
               if (shadow_fury<= diff)
               {
               DoCast(SelectTarget(SELECT_TARGET_RANDOM,1,100,true), spell_shadow_fury);
               shadow_fury = 10000;
               } else shadow_fury-= diff;
